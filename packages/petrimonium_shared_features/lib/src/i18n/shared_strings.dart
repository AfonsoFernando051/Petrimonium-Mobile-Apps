/// Key vocabulary shared by every Petrimonium product that resolves copy
/// through [TranslatorEngine].
///
/// A constant belongs here when both products need to name the *same* concept
/// - it says nothing about the wording. Two products may render
/// [SharedStrings.brandTagline] differently; what they may not do is disagree
/// about the key. Wording lives in `shared_copy.dart` (where the products
/// agree) or in each app's own product catalogue (where they deliberately
/// do not).
///
/// Each app's `AppStrings` aliases these constants rather than repeating the
/// literals, so removing or renaming a key here is a compile error in the apps
/// instead of a string that silently stops resolving.
class SharedStrings {
  SharedStrings._();
