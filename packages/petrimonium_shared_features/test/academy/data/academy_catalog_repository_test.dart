...
void main() {
  ...
  group('fetchAndCache', () {
    test('parses the response and caches its raw body under the language key', () async {
      ...
      verify(() => mockApiClient.get('${AcademyCatalogRepository.catalogEndpoint}?lang=pt')).called(1);
    ...
    test('throws with the backend detail on a non-200 response', () async {
      ...
      ).thenAnswer((_) async => http.Response(jsonEncode({'detail': 'Unsupported lang: fr'}), 400));
    ...
    test('keeps the last valid cache when a successful response has no domains', () async {
      when(() => mockApiClient.get(any())).thenAnswer((_) async => http.Response(jsonEncode(_catalogJson()), 200));
      ...
      await expectLater(() => repository.fetchAndCache('pt'), throwsA(isA<FormatException>()));
  ...
  group('loadCached', () {
    ...
    test('keeps a separate cache entry per language', () async {
      when(
        () => mockApiClient.get('${AcademyCatalogRepository.catalogEndpoint}?lang=pt'),
    ...
    test('returns null instead of throwing when the cached body no longer parses', () async {
    ...
    test('discards a cached catalog with modules but no available schools', () async {
      ...
      SharedPreferences.setMockInitialValues({'academy_catalog_cache_pt': jsonEncode(inconsistentCatalog)});
      repository = AcademyCatalogRepository(apiClient: mockApiClient);
