...
///
/// [mascotController] is optional and only drives the pet's *look*: when
/// given, the avatar renders the pet's Rive character (see
/// [PetRiveCompanion]) and switches to `think` while a reply is being
/// generated (D-007 "Reflection"); without it, or for species with no
/// `Companion`-contract `.riv`, the static portrait is shown as before.
...
class _MentorScreenState extends State<MentorScreen> {
  ...
  /// The pet's face inside the round avatar: the Rive character cropped to
  /// its head (`cover` + `topCenter`) when available, otherwise the static
  /// portrait.
  Widget _petAvatar({required double size, required double iconSize}) {
    final tokens = context.colors;
    Widget portrait(BuildContext _) => Image.asset(
          _petAsset,
          width: size,
          height: size,
          fit: BoxFit.cover,
          errorBuilder: (_, _, _) =>
              Icon(Icons.pets, color: tokens.textSecondary, size: iconSize),
        );
    final mascot = widget.mascotController;
    if (mascot == null) return portrait(context);
    return PetRiveCompanion(
      ...
      // Screen-local mood: "thinking" only here, while the reply is being
      // generated — never pushed into the shared controller, so the
      // header/home pet isn't affected. Otherwise follow the controller.
      stateOverride: _controller.isSending ? PetAnimationState.think : null,
      fallbackBuilder: portrait,
    );
  }
        ...
        children: [
          _avatarWithHalo(
            ...
            avatar: CircleAvatar(
              ...
              child: ClipOval(child: _petAvatar(size: 40, iconSize: 20)),
            ),
          ),
        ...
        children: [
          ...
          _avatarWithHalo(
            ...
            avatar: CircleAvatar(
              radius: 40,
              backgroundColor: tokens.surface,
              child: ClipOval(child: _petAvatar(size: 80, iconSize: 40)),
            ),
          ),
