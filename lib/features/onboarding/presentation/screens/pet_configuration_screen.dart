...
class _PetConfigurationScreenState extends State<PetConfigurationScreen> {
  ...
  /// Only feeds [PetRiveCompanion] here — the species shown is
  /// [_selectedSpecie] (via `specieOverride`), not a saved profile, and this
  /// controller never loads or persists anything.
  late final MascotController _previewMascot = MascotController(repository: DI.mascotRepository);

  /// Brief `happy` reaction right after a species is picked; `null` = idle.
  PetAnimationState? _previewMood;
  Timer? _previewMoodTimer;

  @override
  void dispose() {
    _previewMoodTimer?.cancel();
    _previewMascot.dispose();
  ...
  void _selectSpecie(PetSpecieEnum specie) {
    ...
    setState(() {
      ...
      _previewMood = PetAnimationState.happy;
    });
    _previewMoodTimer = Timer(const Duration(milliseconds: 900), () {
      if (mounted) setState(() => _previewMood = null);
    });
  }
        ...
        children: [
          ...
          Center(
            child: PetHeroCapsule(
              size: 210,
              // Species with a `Companion`-contract Rive character (today:
              // wolf) preview animated; the rest keep their portrait.
              child: PetRiveCompanion(
                controller: _previewMascot,
                specieOverride: _selectedSpecie,
                stateOverride: _previewMood,
                size: 210,
                interactive: false,
