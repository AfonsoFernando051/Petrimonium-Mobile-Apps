...
enum LabNarrativeVariant { introduction, interpretation, takeaway, investingConnection, disclaimer }
...
class LabNarrativeCard extends StatelessWidget {
  const LabNarrativeCard({super.key, required this.text, required this.variant});

  final String text;
  final LabNarrativeVariant variant;

  ({IconData icon, Color accent}) _look() => switch (variant) {
    LabNarrativeVariant.introduction => (icon: Icons.menu_book_outlined, accent: AppColors.goldenBorder),
    LabNarrativeVariant.interpretation => (icon: Icons.insights, accent: AppColors.neonCyan),
    LabNarrativeVariant.takeaway => (icon: Icons.lightbulb_outline_rounded, accent: AppColors.neonViolet),
    LabNarrativeVariant.investingConnection => (icon: Icons.link, accent: AppColors.neonCyan),
    LabNarrativeVariant.disclaimer => (icon: Icons.info_outline, accent: AppColors.subtleText),
  };
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            Expanded(
              child: Text(text, style: AppTextStyles.label.copyWith(color: tokens.textSecondary, height: 1.4)),
            ),
          ],
        ),
      ),
