...
void main() {
  ...
  test('refreshes local progress when a lesson completion event is emitted', () async {
    final controller = buildController();
    await controller.load();
  ...
  group('pending-sync retry', () {
    ...
    test('load() retries a lesson left pending from a previous failed sync', () async {
      ...
      when(
        () => mockRemoteDataSource.completeLesson(testLesson1.id, perfectFirstTry: false),
      ).thenAnswer((_) async => resultFor(testLesson1.id));
      final controller = buildController(remoteDataSource: mockRemoteDataSource);

      await controller.load();

      verify(() => mockRemoteDataSource.completeLesson(testLesson1.id, perfectFirstTry: false)).called(1);
      expect(await repository.loadPendingSyncLessonIds(), isEmpty);
    });

    test('load() retries a perfect lesson with perfectFirstTry: true', () async {
      await repository.markLessonCompleted(testLesson1.id);
      await repository.markLessonPerfect(testLesson1.id);
      await repository.markPendingSync(testLesson1.id);
      when(
        () => mockRemoteDataSource.completeLesson(testLesson1.id, perfectFirstTry: true),
      ).thenAnswer((_) async => resultFor(testLesson1.id));
      final controller = buildController(remoteDataSource: mockRemoteDataSource);

      await controller.load();

      verify(() => mockRemoteDataSource.completeLesson(testLesson1.id, perfectFirstTry: true)).called(1);
    });
    ...
    test('load() does not retry anything when nothing is pending', () async {
      ...
      verifyNever(() => mockRemoteDataSource.completeLesson(any(), perfectFirstTry: any(named: 'perfectFirstTry')));
    });
  });
}
