...
void main() {
  ...
  setUp(() {
    ...
    // on the network.
    when(
      () => mockRemoteDataSource.completeLesson(any(), perfectFirstTry: any(named: 'perfectFirstTry')),
  ...
  Widget buildTestable({required Lesson lesson}) {
    return MaterialApp(
      theme: AppTheme.dark,
      home: LessonScreen(lesson: lesson, catalog: buildAcademyCatalogSnapshot(), mascotController: mascotController),
    );
  }
  ...
  group('LessonScreen', () {
    ...
    testWidgets('a ChoiceQuestionStep only allows advancing once answered', (tester) async {
      ...
      final continueButtonAfterAnswer = tester.widget<GameButton>(find.byType(GameButton));
      expect(continueButtonAfterAnswer.onPressed, isNotNull);
    });

    testWidgets('completing the lesson shows LessonCompleteCard with the earned xp', (tester) async {
      ...
      final completedIds = await DI.academyProgressRepository.loadCompletedLessonIds();
      expect(completedIds, contains(testLesson3.id));
    });

    testWidgets('finishing the final lesson of a module shows the social-share card', (tester) async {
    ...
    testWidgets('tapping Continuar on LessonCompleteCard opens the next lesson in the module', (tester) async {
      await tester.pumpWidget(buildTestable(lesson: testLesson1));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 300));
