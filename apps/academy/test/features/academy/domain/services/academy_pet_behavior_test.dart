...
void main() {
  ...
  group('AcademyPetBehavior.pageEnter — home', () {
    ...
    test('falls back to the review-due nudge when no level-up is imminent but a review is', () {
      final message = behavior.pageEnter(PetContext.home, userXp: 0, data: {'reviewDueCount': '3'});
    ...
    test('falls back to the continue-lesson nudge when no review is due either', () {
      final message = behavior.pageEnter(PetContext.home, userXp: 0, data: {'lessonTitle': 'Renda Fixa Básica'});
    ...
    test('prioritizes the imminent level-up over the continue/review fallback', () {
      final message = behavior.pageEnter(PetContext.home, userXp: 30, data: {'reviewDueCount': '3'});

      expect(message!.id, 'home_xp_to_next_level');
    });

    test('offers nothing when neither a level-up nor review/continue data is available', () {
      final message = behavior.pageEnter(PetContext.home, userXp: 0, data: const {});
      expect(message, isNull);
    });

    test('offers the mission-almost-done nudge when a missionTitle is supplied', () {
      final message = behavior.pageEnter(PetContext.home, userXp: 0, data: {'missionTitle': 'Aula do Dia'});
    ...
    test('the mission-almost-done nudge outranks the imminent level-up nudge', () {
      final message = behavior.pageEnter(PetContext.home, userXp: 30, data: {'missionTitle': 'Aula do Dia'});

      expect(message!.id, 'home_mission_almost_done');
    });

    test('offers a return-after-inactivity greeting once the gap reaches the sleep threshold', () {
      final message = behavior.pageEnter(PetContext.home, userXp: 0, data: {'daysSinceLastSession': '3'});
    ...
    test('a gap shorter than the sleep threshold does not trigger the return greeting', () {
      final message = behavior.pageEnter(PetContext.home, userXp: 0, data: {'daysSinceLastSession': '2'});
  ...
  group('AcademyPetBehavior.pageEnter — academy', () {
    ...
    test('offers a continue-lesson nudge when there is a lesson title and no review due', () {
      final message = behavior.pageEnter(PetContext.academy, userXp: 0, data: {'lessonTitle': 'Renda Fixa'});
    ...
    test('offers nothing when lessonTitle is an empty string', () {
      final message = behavior.pageEnter(PetContext.academy, userXp: 0, data: {'lessonTitle': ''});
      expect(message, isNull);
    });
  });
  ...
  group('AcademyPetBehavior.pageEnter — contexts it does not own', () {
    test('offers nothing for portfolio/mentor/profile', () {
      for (final context in [PetContext.portfolio, PetContext.mentor, PetContext.profile]) {
  ...
  group('AcademyPetBehavior — event-triggered reactions', () {
    ...
    test('difficultyDetected is per-school and normal priority', () {
      final message = behavior.onEvent(const DifficultyDetectedEvent('Renda Fixa'));
    ...
    test('labSimulatorCompleted is per-simulator and high priority', () {
      final message = behavior.onEvent(const FinancialLabSimulatorCompletedEvent('Juros Compostos'));
  ...
  group('AcademyPetBehavior.questionFeedbackTitle', () {
    test('picks a deterministic title from the correct/incorrect pool', () {
      final correct = AcademyPetBehavior.questionFeedbackTitle(correct: true, seed: 0);
      final incorrect = AcademyPetBehavior.questionFeedbackTitle(correct: false, seed: 0);
