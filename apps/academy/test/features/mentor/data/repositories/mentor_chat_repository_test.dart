...
void main() {
  ...
  group('loadConversation / listConversations / renameConversation / deleteConversation', () {
    ...
    test('loadSuggestedPrompts delegates to the data source', () async {
      when(() => mockDataSource.getSuggestedPrompts(language: 'pt')).thenAnswer((_) async => ['O que é um ETF?']);
  ...
  group('sendMessage', () {
    test('sends the message with the user\'s saved goal/horizon and current language as context', () async {
      ...
      await petPreferencesRepository.saveHorizon(InvestmentHorizonEnum.moreThanFiveYears);
      when(
      ...
      expect(captured['investmentHorizon'], InvestmentHorizonEnum.moreThanFiveYears.label);
    ...
    test('returns the result from the data source', () async {
      when(
        ...
        (_) async => const MentorChatResult(reply: 'Claro, posso ajudar!', conversationId: 5, title: 'Dúvida'),
      );
