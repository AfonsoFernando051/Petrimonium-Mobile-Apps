...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockMascotRepository.loadProfile()).thenAnswer((_) async => PetProfile());
  ...
  group('PortfolioChoiceScreen', () {
    testWidgets('explains the learn-first journey without asking for assets', (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(theme: AppTheme.dark, home: const PortfolioChoiceScreen()));
      ...
      expect(find.text('Sua carteira entra quando fizer sentido'), findsOneWidget);
    ...
    testWidgets('shows the pet-name footnote once the mascot profile loads with a name', (WidgetTester tester) async {
    ...
    testWidgets('shows no footnote when the mascot has no name yet', (WidgetTester tester) async {
      when(() => mockMascotRepository.loadProfile()).thenAnswer((_) async => PetProfile(name: null));
    ...
    testWidgets('continue action resolves the optional portfolio step', (WidgetTester tester) async {
