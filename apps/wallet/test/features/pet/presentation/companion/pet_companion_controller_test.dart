...
void main() {
  ...
  group('enterContext', () {
    test('offers the home nudge once the user is past halfway to the next level', () async {
    ...
    test('offers nothing when the user just leveled up (progress near zero)', () async {
    ...
    test('allowAmbientFallback: false (the default) stays silent with nothing else to say', () async {
      mascotRepository.profileToReturn = PetProfile(xp: 0);
      await mascotController.loadProfile();
    ...
    test('allowAmbientFallback: true offers the motivational fallback with nothing else to say', () async {
      mascotRepository.profileToReturn = PetProfile(xp: 0);
      await mascotController.loadProfile();
    ...
    test('allowAmbientFallback: true substitutes the motivational fallback once the real '
        'nudge is cooling down, instead of going silent', () async {
      mascotRepository.profileToReturn = PetProfile(xp: 0);
      await mascotController.loadProfile();
    ...
    test('a page-enter nudge never interrupts a message already showing', () async {
      mascotRepository.profileToReturn = PetProfile(xp: 30);
      await mascotController.loadProfile();
    ...
    test('a mission one lesson away from completion outranks the near-level-up nudge', () async {
    ...
    test('a returning-after-inactivity greeting outranks everything else in _homeNudge', () async {
      mascotRepository.profileToReturn = PetProfile(xp: 30);
      await mascotController.loadProfile();

      controller.enterContext(PetContext.home, data: {'missionTitle': 'Aula do Dia', 'daysSinceLastSession': '5'});

      expect(controller.currentMessage?.id, startsWith('home_return_greeting_'));
    });

    test('a gap shorter than the sleep threshold does not trigger the return greeting', () async {
      mascotRepository.profileToReturn = PetProfile(xp: 30);
      await mascotController.loadProfile();
  ...
  group('app events', () {
    test('a level-up reaction replaces a lower-priority nudge already showing', () async {
      mascotRepository.profileToReturn = PetProfile(xp: 30);
      await mascotController.loadProfile();
      controller.enterContext(PetContext.home);
    ...
    test('a same-tick normal event cannot clobber a just-shown high-priority celebration', () async {
      AppEventBus.instance.emit(const UserLeveledUpEvent(3));
      await flushMicrotasks();
      expect(controller.currentMessage?.id, 'event_level_up');

      AppEventBus.instance.emit(const XpGainedEvent(amount: 10, newTotalXp: 40));
      await flushMicrotasks();
    ...
    test('a high-concentration event carries the real ticker/percent', () async {
      AppEventBus.instance.emit(const HighConcentrationDetectedEvent(ticker: 'PETR4', percent: 55));
      await flushMicrotasks();
