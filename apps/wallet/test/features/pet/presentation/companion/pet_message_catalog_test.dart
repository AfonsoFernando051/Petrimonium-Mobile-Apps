...
void main() {
  group('PetMessageCatalog.pageEnter — home', () {
    test('offers the XP-to-next-level nudge once past halfway to the next level', () {
      // Level 1->2 needs 50 total XP; 30 is 60% of the way there.
      final message = PetMessageCatalog.pageEnter(PetContext.home, userXp: 30);
    ...
    test('offers nothing when progress toward the next level is below half', () {
      final message = PetMessageCatalog.pageEnter(PetContext.home, userXp: 0);
      expect(message, isNull);
    });

    test('offers nothing right at a level-up boundary (progress resets to 0)', () {
      // Level 1->2 needs 50 total XP; landing exactly on it resets progress
      // toward level 2->3, which is also below half.
      final message = PetMessageCatalog.pageEnter(PetContext.home, userXp: 50);
      expect(message, isNull);
    });

    test('falls back to the review-due nudge when no level-up is imminent but a review is', () {
      final message = PetMessageCatalog.pageEnter(PetContext.home, userXp: 0, data: {'reviewDueCount': '3'});
    ...
    test('falls back to the continue-lesson nudge when no review is due either', () {
    ...
    test('prioritizes the imminent level-up over the continue/review fallback', () {
      final message = PetMessageCatalog.pageEnter(PetContext.home, userXp: 30, data: {'reviewDueCount': '3'});

      expect(message!.id, 'home_xp_to_next_level');
    });

    test('offers nothing when neither a level-up nor review/continue data is available', () {
      final message = PetMessageCatalog.pageEnter(PetContext.home, userXp: 0, data: const {});
      expect(message, isNull);
    });

    test('offers the mission-almost-done nudge when a missionTitle is supplied', () {
      final message = PetMessageCatalog.pageEnter(PetContext.home, userXp: 0, data: {'missionTitle': 'Aula do Dia'});
    ...
    test('the mission-almost-done nudge outranks the imminent level-up nudge', () {
      final message = PetMessageCatalog.pageEnter(
        PetContext.home,
    ...
    test('offers a return-after-inactivity greeting once the gap reaches the sleep threshold', () {
      final message = PetMessageCatalog.pageEnter(PetContext.home, userXp: 0, data: {'daysSinceLastSession': '3'});

      expect(message, isNotNull);
      expect(message!.id, startsWith('home_return_greeting_'));
      expect(message.textKey, anyOf(AppStrings.companionHomeReturnGreeting1, AppStrings.companionHomeReturnGreeting2));
    });

    test('the return greeting outranks the mission-almost-done and level-up nudges', () {
      final message = PetMessageCatalog.pageEnter(
        PetContext.home,
        userXp: 30,
    ...
    test('a gap shorter than the sleep threshold does not trigger the return greeting', () {
      final message = PetMessageCatalog.pageEnter(PetContext.home, userXp: 0, data: {'daysSinceLastSession': '2'});
  ...
  group('PetMessageCatalog.homeMotivationalFallback', () {
    ...
    test('is always one of the known pool entries, low priority, and content-free', () {
  ...
  group('PetMessageCatalog.pageEnter — academy', () {
    test('offers a review-due nudge when reviewDueCount > 0, taking priority over a continue nudge', () {
      final message = PetMessageCatalog.pageEnter(
        PetContext.academy,
        userXp: 0,
    ...
    test('offers a continue-lesson nudge when there is a lesson title and no review due', () {
      final message = PetMessageCatalog.pageEnter(PetContext.academy, userXp: 0, data: {'lessonTitle': 'Renda Fixa'});
    ...
    test('offers nothing when there is no review due and no lesson title', () {
      final message = PetMessageCatalog.pageEnter(PetContext.academy, userXp: 0);
      expect(message, isNull);
    });

    test('offers nothing when lessonTitle is an empty string', () {
      final message = PetMessageCatalog.pageEnter(PetContext.academy, userXp: 0, data: {'lessonTitle': ''});
      expect(message, isNull);
    });
  });
  ...
  group('PetMessageCatalog.pageEnter — portfolio', () {
    test('offers a diversification nudge when spread across more than 1 asset', () {
      final message = PetMessageCatalog.pageEnter(PetContext.portfolio, userXp: 0, data: {'count': '3'});
    ...
    test('offers nothing at exactly 1 holding — celebrated separately by firstInvestment()', () {
      expect(PetMessageCatalog.pageEnter(PetContext.portfolio, userXp: 0, data: {'count': '1'}), isNull);
    });

    test('offers the activation nudge at 0 holdings (missing count)', () {
      final message = PetMessageCatalog.pageEnter(PetContext.portfolio, userXp: 0);
    ...
    test('treats an unparseable count as 0, offering the activation nudge', () {
      final message = PetMessageCatalog.pageEnter(PetContext.portfolio, userXp: 0, data: {'count': 'not-a-number'});
      expect(message, isNotNull);
      expect(message!.id, 'portfolio_activation_nudge');
    });
  });
  ...
  group('PetMessageCatalog.investorStatusReaction', () {
    test('returns the already-invests key when true', () {
      expect(PetMessageCatalog.investorStatusReaction(alreadyInvests: true), AppStrings.companionInvestorStatusYes);
    });

    test('returns the not-yet key when false', () {
      expect(PetMessageCatalog.investorStatusReaction(alreadyInvests: false), AppStrings.companionInvestorStatusNo);
    });
  });
  ...
  group('PetMessageCatalog.pageEnter — mentor and profile', () {
    ...
    test('profile always offers a level/stage summary', () {
      final message = PetMessageCatalog.pageEnter(PetContext.profile, userXp: 60);
      expect(message, isNotNull);
