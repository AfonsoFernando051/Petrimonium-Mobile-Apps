...
void main() {
  group('Money', () {
    ...
    test('rejects excess precision instead of rounding it', () {
      expect(() => Money.fromDecimal('10.001', CurrencyCode.eur), throwsFormatException);
    });
    ...
    test('never adds different currencies', () {
      ...
      expect(() => brl.compareTo(eur), throwsA(isA<CurrencyMismatchException>()));
    });
    ...
    test('installments add back to the original amount exactly', () {
      ...
      expect(installments.map((item) => item.minorUnits), [33334, 33333, 33333]);
      expect(installments.fold(Money.zero(CurrencyCode.eur), (sum, item) => sum + item), total);
    });
  });

  group('localized money input and output', () {
    String normalized(String value) => value.replaceAll('\u00a0', ' ').replaceAll('\u202f', ' ');

    test('parses Brazilian and Portuguese manual input without doubles', () {
      expect(
        MoneyInput.tryParse('R\$ 1.234,56', currency: CurrencyCode.brl, locale: const Locale('pt', 'BR')),
        Money.fromDecimal('1234.56', CurrencyCode.brl),
      );
      expect(
        MoneyInput.tryParse('1 234,56 €', currency: CurrencyCode.eur, locale: const Locale('pt', 'PT')),
    ...
    test('formats BRL and EUR with the selected locale', () {
      final brl = normalized(
        MoneyFormat.currency(Money.fromDecimal('1234.56', CurrencyCode.brl), const Locale('pt', 'BR')),
      );
      final eur = normalized(
        MoneyFormat.currency(Money.fromDecimal('1234.56', CurrencyCode.eur), const Locale('pt', 'PT')),
      );
