...
final class RegionalSuggestion {
  const RegionalSuggestion({required this.currency, required this.locale});
...
RegionalSuggestion suggestionFor(CountryCode country) => switch (country) {
  CountryCode.brazil => const RegionalSuggestion(currency: CurrencyCode.brl, locale: InterfaceLocale.ptBr),
  CountryCode.portugal => const RegionalSuggestion(currency: CurrencyCode.eur, locale: InterfaceLocale.ptPt),
};
...
final class HealthProfile {
  ...
  HealthProfile copyWith({
    ...
    currencyChangeAllowed: currencyChangeAllowed ?? this.currencyChangeAllowed,
  );
}
