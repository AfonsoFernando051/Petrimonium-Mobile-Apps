...
final class MentorReply {
  const MentorReply({required this.reply, required this.conversationId, required this.title, required this.sources});

  factory MentorReply.fromJson(Map<String, dynamic> json) => MentorReply(
    reply: json['reply'] as String? ?? '',
    conversationId: (json['conversationId'] as num?)?.toInt(),
    title: json['title'] as String?,
    sources: ((json['sources'] as List?) ?? const []).map((item) => item.toString()).toList(growable: false),
  );
